...
export const GoogleSheetsIntegration = {
  ...
  SCRIPT_URL:
    'https://script.google.com/macros/s/AKfycbxB7pmLy4x5eUc8bXgbN3mQbI3AAZFWAuRh-Dbdh6H69YvUBBQ4bkQ0SFrTzSGKm2Ps2w/exec',
  ...
  collectFormData: function () {
    ...
    formData.append(
      'hasWantedItems',
      this.convertToJapanese(hasWantedItemsValue, 'hasWantedItems')
    );
    ...
    formData.append(
      'wantedQuantitySuit',
      form.querySelector('[name="wantedQuantitySuit"]')?.value || ''
    );
    formData.append(
      'wantedQuantityJacket',
      form.querySelector('[name="wantedQuantityJacket"]')?.value || ''
    );
    formData.append(
      'wantedQuantityShirt',
      form.querySelector('[name="wantedQuantityShirt"]')?.value || ''
    );
    formData.append(
      'wantedQuantityTshirt',
      form.querySelector('[name="wantedQuantityTshirt"]')?.value || ''
    );
    formData.append(
      'wantedQuantityPants',
      form.querySelector('[name="wantedQuantityPants"]')?.value || ''
    );
    formData.append(
      'wantedQuantityShoes',
      form.querySelector('[name="wantedQuantityShoes"]')?.value || ''
    );
    formData.append(
      'wantedQuantityOuter',
      form.querySelector('[name="wantedQuantityOuter"]')?.value || ''
    );
    formData.append(
      'wantedQuantityBelt',
      form.querySelector('[name="wantedQuantityBelt"]')?.value || ''
    );

    // Investment Amount
    const investmentBudgetValue =
      form.querySelector('[name="investmentBudget"]:checked')?.value || '';
    formData.append(
      'investmentBudget',
      this.convertToJapanese(investmentBudgetValue, 'investmentBudget')
    );
    ...
    formData.append(
      'favoredBrandsOther',
      form.querySelector('[name="favoredBrandsOther"]')?.value || ''
    );
    ...
    formData.append(
      'bodyMaleConcerns',
      this.convertToJapanese(bodyMaleConcerns, 'bodyMaleConcerns')
    );

    const bodyFemaleConcerns = Array.from(
      form.querySelectorAll('[name="bodyFemaleConcerns"]:checked')
    )
      .map(cb => cb.value)
      .join(', ');
    formData.append(
      'bodyFemaleConcerns',
      this.convertToJapanese(bodyFemaleConcerns, 'bodyFemaleConcerns')
    );

    formData.append(
      'bodyOtherConcerns',
      form.querySelector('[name="bodyOtherConcerns"]')?.value || ''
    );

    // Service Expectations
    const serviceExpectations = Array.from(
      form.querySelectorAll('[name="serviceExpectations"]:checked')
    )
      .map(cb => cb.value)
      .join(', ');
    formData.append(
      'serviceExpectations',
      this.convertToJapanese(serviceExpectations, 'serviceExpectations')
    );
    ...
    const proposalFrequencyValue =
      form.querySelector('[name="proposalFrequency"]:checked')?.value || '';
    formData.append(
      'proposalFrequency',
      this.convertToJapanese(proposalFrequencyValue, 'proposalFrequency')
    );
  ...
  saveToLocal: function (formData) {
    try {
      ...
      dataObj.facePhotoUploaded =
        facePhoto && facePhoto instanceof File && facePhoto.size > 0
          ? 'アップロード済み'
          : '未アップロード';
      dataObj.bodyPhotoUploaded =
        bodyPhoto && bodyPhoto instanceof File && bodyPhoto.size > 0
          ? 'アップロード済み'
          : '未アップロード';
