// Style Patterns Handler - Single Style with 3×5 = 15 patterns display
...
const stylePatternData = {
    ...
    minimal: {
        ...
        patterns: [
            ...
            { id: 'minimal-7', name: 'ニュートラルトーン', image: 'style-minimal-7.jpg' },
...
function updatePatternDisplay(container, wrapper) {
    ...
    // Set the first selected style as current if none is set
    if (!currentStyleKey || !selectedStyles.has(currentStyleKey)) {
        currentStyleKey = Array.from(selectedStyles)[0];
    }

    // Create style switcher if multiple styles are selected
    if (selectedStyles.size > 1) {
        const switcherContainer = document.createElement('div');
        switcherContainer.className = 'style-switcher-container';
        switcherContainer.innerHTML = `
            <h4>スタイルを切り替え</h4>
            <div class="style-switcher-tabs">
                ${Array.from(selectedStyles).map(styleKey => {
                    const styleData = stylePatternData[styleKey];
                    return `
                        <button type="button"
                                class="style-tab ${styleKey === currentStyleKey ? 'active' : ''}"
                                data-style="${styleKey}">
                            ${styleData ? styleData.name : styleKey}
                        </button>
                    `;
                }).join('')}
            </div>
        ...
        switcherContainer.querySelectorAll('.style-tab').forEach(tab => {
            tab.addEventListener('click', function() {
                currentStyleKey = this.dataset.style;
                updatePatternDisplay(container, wrapper);
            });
        });
    }

    // Display patterns for current style
    const styleData = stylePatternData[currentStyleKey];
    if (!styleData) return;

    const patternSection = document.createElement('div');
    patternSection.className = 'single-style-pattern-section';
    patternSection.innerHTML = `
        <div class="pattern-section-header">
            <h3 class="pattern-section-title">${styleData.name} - パターン選択</h3>
            <p class="pattern-section-description">15のパターンから複数選択可能です</p>
        </div>
        <div class="style-patterns-grid-3x5" id="patterns-${currentStyleKey}">
            ${styleData.patterns.map((pattern, i) => {
                ...
                <div class="style-pattern-card" data-row="${row}" data-col="${col}">
                    <input type="checkbox"
                           name="stylePattern_${currentStyleKey}"
                           value="${pattern.id}"
                           id="${pattern.id}">
...
function clearCurrentStylePatterns() {
    if (!currentStyleKey) return;

    const patternCheckboxes = document.querySelectorAll(`input[name="stylePattern_${currentStyleKey}"]`);
